...
xmlns:doc="http://nwalsh.com/xsl/documentation/1.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:exsl="http://exslt.org/common" version="1.0" exclude-result-prefixes="doc d exsl">

<!-- CUSTOMIZATION 2017-07-03
     The distributed chunktoc.xsl in v1.79.1 is broken, missing the exsl
     namespace declaration. This makes it difficult to extend the default
     stylesheets cleanly. Here's how we try to do it:
     We make a copy of the default chunktoc.xsl to our customization layer
     (this is that copy).
     In the copy, we make two changes:
     1) We add the missing namespace declaration.
     2) We change the import hrefs from file-relative to use URIs.
        These imports become the bridge into the default stylesheet distribution.

    This allows us to keep actual customizations in our own chunktoc.xsl, and
    include this patched copy like we would normally import the original.

    See also: https://github.com/docbook/xslt10-stylesheets/issues/32
     -->
